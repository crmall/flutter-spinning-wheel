...
class SpinVelocity {
  ...
  double getVelocity(Offset position, Offset pps) {
    final cuadrantIndex = _getCuadrantFromOffset(position);
    final cuadrant = cuadrants[cuadrantIndex];
    if (cuadrant != null) {
  ...
  int _getCuadrantFromOffset(Offset p) => p.dx > width_0_5 ? (p.dy > height_0_5 ? 2 : 1) : (p.dy > height_0_5 ? 3 : 4);
  ...
  double _normalizeAngle(double angle) => angle > 0 ? (angle > pi_0_5 ? (pi_2_5 - angle) : (pi_0_5 - angle)) : pi_0_5 - angle;
